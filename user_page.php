...
if (isset($_POST['other_id']) && $_POST['other_id'] != 0) {
  $other_id = trim($_POST['other_id']);
  $arr = userinfo_query($other_id,$conn);
  $sql_concerned = "select count(*) from follow where by_follower = $other_id and fans = $uid";
  $query = $conn->query($sql_concerned);
  $check_fans = $query->fetch(PDO::FETCH_NUM);
  $arr['is_concerned'] = ($check_fans[0] == 0)?'no':'yes';

  $sql_fans = "select count(*) from follow where by_follower = $uid and fans = $other_id";
  $query = $conn->query($sql_fans);
  $check_fans = $query->fetch(PDO::FETCH_NUM);
  $arr['is_fans'] = ($check_fans[0] == 0)?'no':'yes';
}else{
