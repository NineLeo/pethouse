...
$sql = "select statuses.*,user.* from statuses,user where statuses.uid = user.uid order by statuses.mid desc limit $count";
if ($_GET['since_id']!=0 && $_GET['max_id']==0) {
	$since_id = trim($_GET['since_id']);
	$sql = "select statuses.*,user.* from statuses,user where statuses.uid = user.uid and mid>$since_id order by statuses.mid  limit $count ";
}elseif ($_GET['since_id']==0 && $_GET['max_id']!=0) {
	$max_id = trim($_GET['max_id']);
	$sql = "select statuses.*,user.* from statuses,user where statuses.uid = user.uid and mid<$max_id order by statuses.mid desc limit $count";
}
